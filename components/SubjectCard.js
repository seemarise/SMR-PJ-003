...
export default function SubjectCard({ title, date, imageSrc = "/book.png", onClick }) {
  return (
    <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 to-blue-700 text-white rounded-2xl p-4 shadow-lg">
      ...
      <div className="absolute bottom-4 right-4 flex items-center justify-center">
        <div className="relative w-20 h-20">
          {/* Back square */}
          <div className="absolute top-2 left-2 w-16 h-16 bg-gray-800 rounded-lg transform rotate-3 flex items-center justify-center">
            <span className="text-white text-sm font-bold opacity-70">
              {title === "English" ? "u" : "B.C"}
            </span>
          </div>
          {/* Front square */}
          <div className="absolute top-0 left-0 w-16 h-16 bg-gray-700 rounded-lg transform -rotate-3 flex items-center justify-center shadow-xl">
            <span className="text-white text-base font-bold">
              {title === "English" ? "Abc" : "A.D."}
            </span>
          </div>
        </div>
      </div>
